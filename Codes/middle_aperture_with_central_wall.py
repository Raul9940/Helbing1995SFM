...
def initial_conditions(n, width, length, p):
    ...
    for i in range(n):
        ...
        old[3, i]=0.0 #Initial y velocity y
           
    return old, a



def position_evolver(a, old, i, delta_t, tau, width, max_v_coef, pos_int_wall, widthwall, sigma, v0, r, u0, c, phi, hRadius, intWidth):
    n=np.ma.size(a,1)
    new=np.zeros(4)
    rabx=np.zeros(n)
    raby=np.zeros(n)
    ...
    #Goal force
        if (a[3, i]==True and a[0,i]==1.0) or (a[3, i]==False and a[0,i]==-1.0):
            if old[1, i]<width/2.0-widthwall/2.0+hRadius:
                ry_wall=width/2.0-widthwall/2.0+hRadius
            elif old[1, i]>width/2.0-intWidth/2.0-hRadius:
                ry_wall=width/2.0-widthwall/2.0-hRadius
            else:
                ry_wall=old[1, i]    
        elif (a[3, i]==False and a[0,i]==1.0) or (a[3, i]==True and a[0, i]==-1.0):     
            if old[1, i]<width/2.0+intWidth/2.0+hRadius:
                ry_wall=width/2.0+intWidth/2.0+hRadius
            elif old[1, i]>width/2.0+widthwall/2.0-hRadius:
                ry_wall=width/2.0+intWidth/2.0+hRadius
            else:
                ry_wall=old[1, i]
        elif widthwall/2.0-intWidth/2.0<2.0*hRadius:
        ...
        new[2]=delta_t*(1.0/tau*(a[2, i]*ex-old[2, i])) #Term 1
        ...
        if old[1, i]>=widthwall/2.0-intWidth/2.0 and old[1, i]<=widthwall/2.0+intWidth/2.0:
            distIntMidx=old[0,i]-pos_int_wall
            distIntMidy=0.0
        elif old[1, i]<widthwall/2.0-intWidth/2.0:
            distIntMidx=old[0,i]-pos_int_wall
            distIntMidy=old[1, i]-(width-intWidth)/2.0
        else:
            distIntMidx=old[0,i]-pos_int_wall
            distIntMidy=old[1, i]-(width+intWidth)/2.0
        
        distIntMidMod=np.sqrt(distIntMidx**2.0+distIntMidy**2.0)    
        wallsIntMidx=u0/r*np.e**((-distIntMidMod)/r)*distIntMidx/distIntMidMod
        wallsIntMidy=u0/r*np.e**((-distIntMidMod)/r)*distIntMidy/distIntMidMod
        
        new[2]+=delta_t*wallsIntMidx
        new[3]+=delta_t*wallsIntMidy
        ...
        if np.sqrt(new[2]**2.0+new[3]**2.0)>max_v_coef*a[2, i]:
            ...
            new[3]=new[3]*max_v_coef*a[2, i]/np.sqrt(new[2]**2.0+new[3]**2.0)
                ...
                #Pasamos a calcular ahora las posiciones nuevas
...
for people in peopleList:
    for probability in probabilityList:
        ...
        old, init=initial_conditions(n, width, length, p)
        
        new=np.zeros((4, n))
        
        for l in range(t_step):
            for i in range(n):
                new[:,i]=position_evolver(init, old, i, t_delta, tau, width, max_v_coef, pos_int_wall, widthwall, sigma, v0, r, u0, c, phi, hRadius, intWidth)
            ...
            if (l*t_delta)%1.0==0.0:
                ...
                wallInt1, wallInt2=[pos_int_wall, pos_int_wall], [width/2.0-intWidth/2.0, width/2.0+intWidth/2.0]
                plt.plot(wallSup1, wallSup2, wallInf1, wallInf2, wallInt1, wallInt2, color='black')
                plt.xlim(0, length)
                plt.ylim(0,width)#plt.ylim(width/2.0-widthwall/2.0-1.0, width/2.0+widthwall/2.0+1.0)
            ...
            if np.count_nonzero(npazules[:,0]<length/2.0)==0 and np.count_nonzero(nprojos[:,0]>length/2.0)==0:
                print(l*t_delta)
                print('All have crossed the perture')
                break
            for i in range(4):
                for j in range(n):
                    old[i,j]=new[i,j]    
